...
def main(args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for x, y in loader:
            ...
            logits = tpn_model(generated_embeddings, ungenerated_embeddings, hidden_states[:,i,:].unsqueeze(1), labels)
            ...
            # 这个熵写的不对
            #entropy = -(probs * torch.log(probs + 1e-6)).sum(dim=2)

            #entropy[:,:(i+1)] = -1e9
            #_, idx = torch.topk(entropy, k=1, dim=-1)
            ...
            max_probs[:, : (i+1)] = -1.0
            k = 1
            _, idx = torch.topk(max_probs, k=k, dim=-1)


            # 根据 idx 取标签
            z_labels = torch.gather(z_indices, dim=1, index=idx).squeeze(1)

            # 根据 idx 取 logits 分布
            idx_expanded = idx.unsqueeze(-1).expand(-1, -1, logits.size(-1))
            pred_logits = torch.gather(logits, dim=1, index=idx_expanded).squeeze(1)

            # 计算 loss   交叉熵函数包括softmax之前不能再softmax

            loss = F.cross_entropy(pred_logits, z_labels)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--results-dir", type=str, default="results_v3")
